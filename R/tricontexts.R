...
tricontexts <- function(sites, fasta, quiet = FALSE) {
  ...
  # Pre-convert chromosomes to strings for fast substring access
  chrom_strings <- list()
  for (chr in unique(sites$CHROM)) {
    if (chr %in% names(fasta)) {
      chrom_strings[[chr]] <- toupper(paste0(fasta[[chr]], collapse = ""))
    }
  }

  # create progress bar
  if (!quiet) pb <- txtProgressBar(min = 0, max = nrow(sites), style = 3)
  ...
  contexts <- sapply(seq_len(nrow(sites)), function(i) {
    ...
    if(!(chr %in% names(chrom_strings))){
      return(NA)
    }

    chr_str <- chrom_strings[[chr]]
    if(pos > nchar(chr_str)){
      stop(paste("row",i,"POS is larger than chromosome length in fasta"))
    }
    up <- substr(chr_str, pos - 1, pos - 1)
    down <- substr(chr_str, pos + 1, pos + 1)
    ...
    if(!context %in% SBS){
      ...
      up_comp <- toupper(comp(up))
      down_comp <- toupper(comp(down))
      context <- paste0(down_comp,"[",ref,">",alt,"]", up_comp)
      mutation <- paste0(down_comp,ref,up_comp,">", down_comp,alt,up_comp)
    }
