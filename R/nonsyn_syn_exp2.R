...
#'
#'
nonsyn_syn_exp2 <- function(muts, genome, CDS, reps, n) {
  ...
  bp_freq<-data.table(table(REF=genome$Chr1))[REF!="n"]
  ...
  for (rep in 1:reps) {
    ...
    for (i in 1:n) {
      ...
      cds_bp_freq<-data.table(table(REF=random_seq))[REF!="N"]
      cds_bp_freq<-merge(cds_bp_freq, ref_probsdt, by="REF")
      cds_bp_freq$REL_PROB<-prop.table(cds_bp_freq$N*cds_bp_freq$PROB)
      
      selected_ref <- sample(cds_bp_freq$REF, 1, prob = cds_bp_freq$REL_PROB)
